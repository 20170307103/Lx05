   ...
   public class Activity
    {
        ...
        public Model.Activity GetRecommend()
        {
            using(IDbConnection cn = new MySqlConnection(cns))
                {
                string sql = "select * from activity where activityVerify='审核通过' andrecommend='是' and endTime>=now() order by recommendTime desc limit 1";
        ...
        public int UpdateRecommend(Model.Activity activity)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update zctivity set recommend=@recommend，recommendTime=@recommendTime where activityid=@activityId";
